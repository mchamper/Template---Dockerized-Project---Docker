...
export function authCanGuard(guardName: TAuthGuardName, matchAll: boolean = false): CanActivateFn {
  return (next: ActivatedRouteSnapshot, state: RouterStateSnapshot) => {
    ...
    const can = !permissions.length || !!authS.guard(guardName).activeSession()?.can(permissions, matchAll);
